...
public class Astar implements AIModule
{
    static double getHeuristic(final TerrainMap map, final Point pt1, final Point pt2) {
        ...
        double x_diff = Math.pow(2, pt1.x - pt2.x);
        double y_diff = Math.pow(2, pt1.y - pt2.y);
        double h_diff = Math.pow(2, h1 - h2);
        double sqrtt = Math.sqrt(x_diff+y_diff+h_diff);

        return Math.pow(2, sqrtt);
    }
