...
public class minimax_ash_bo extends AIModule
{
    private int player;
    ...
    public void getNextMove(final GameStateModule state)
    {
        ...
        for(int move : moves)
        {
            if(state.canMakeMove(move))//if  move valid
            {
                if(terminate == true)
                {
                    System.out.print("minimax terminate\n");
                    return;
                }
    ...
    private int MaxVal(final GameStateModule state, int depth)
    {
        ...
        for(int move : moves)
        {
            if(terminate == true)
            {
                System.out.print("minimax terminate max\n");
            ...
            {
                state.makeMove(move);
                util = Math.max(util, MinVal(state, depth + 1));
                state.unMakeMove();
            }
        }
        return util;
    ...
    private int MinVal(final GameStateModule state, int depth)
    {
        ...
        for(int move : moves)
        {
            if(terminate == true)
            {
                System.out.print("minimax terminate min\n");
                break;
            }
            ...
            {
                state.makeMove(move);
                util = Math.min(util, MaxVal(state, depth + 1));
                state.unMakeMove();
            }
        }
        return util;
