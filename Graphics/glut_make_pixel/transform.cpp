//created by Xie Zhou on 10/10/2017
...
vector<vector<vector<int> > > rasterize(vector<vector<vector<int> > > vertices, vector<vector<vector<int> > > lined_polygons);
vector<vector<int> > unique_points(vector<vector<int> > list);
void clip_polygon(int x_min, int x_max, int y_min, int y_max, vector<vector<vector<int> > > lines, vector<vector<int> > vert);
int x_intersect(vector<vector<int> > line, int x);
int y_intersect(vector<vector<int> > line, int y);
void save_points_to_file();
void make_polygons();
void draw_polygons();
void make_boards(vector<vector<vector<int> > > list);
void loadPoints();
...
vector<vector<vector<int> > > lined_board; //polygons in a big matrix form 
vector<vector<vector<int> > > filled_polygons; //rasterization part
...
int main(int argc, char **argv)
{
    ...
    vector<int> temp_line(grid_height,0);
    for(int j = 0; j < 3; j++){
        vector<vector<int> > temp_board;
        for(int i = 0; i < grid_width; i++){
            temp_board.push_back(temp_line);
        }
        lined_board.push_back(temp_board);
    }
...
void createGLUTMenus() {
    ...
    glutAddMenuEntry("show VERTICES",VERTICES);
    ...
    glutAddMenuEntry("--Choose polygon 0/1/2--",FINISHED);
    ...
    glutAddMenuEntry("Set rotation angle (in degrees)",ROTATE);
...
void loadPoints(){
    ...
    filenames.push_back("polygon0.data");
    filenames.push_back("polygon1.data");
    filenames.push_back("polygon2.data");
    ...
    for(int i = 0; i < 3; i++){
        ...
        for(int j = 0; j < pointNum; j++){
            ...
            temp_point.push_back(int(x));
            temp_point.push_back(int(y));
...
void display()
{
    ...
    if(which_alg == 0){
        ...
        */
        ...
        if(set_rasterize && !stop_rasterize){
            filled_polygons = rasterize(point_lists, lined_board);
            for(int i = 0; i < 3; i++)
                draw_points(filled_polygons[i]);
        }
    }
    ...
    if(set_clip){
        ...
        clip_polygon(x_min,x_max,y_min,y_max,lines_in_polygon[which_ploy],point_lists[which_ploy]);

        //point_lists[which_ploy]=new_vertices;
...
vector<vector<vector<int> > > rasterize(vector<vector<vector<int> > > vertices, vector<vector<vector<int> > > lined_board){
    ...
    for(int i = 0; i < 3; i++){
        //cout << "polygon: "<< i << endl;
        vector<vector<int> > temp_point_list;//one extrema list per polygon
...
vector<vector<int> > delete_middle_point(vector<vector<int> > list){
    if(list.size() > 2){
        int i = 2;
        int list_size = list.size();
        while(i < list_size){
            if(list[i][0] == list[i-1][0] && list[i-1][0] == list[i-2][0]){
                list.erase(list.begin()+i-1);
                i--;
            }
            else if(list[i][1] == list[i-1][1] && list[i-1][1] == list[i-2][1]){
                list.erase(list.begin()+i-1);
                i--;
            }
            else{}
...
void clip_polygon(int x_min, int x_max, int y_min, int y_max, vector<vector<vector<int> > > lines, vector<vector<int> > vert){
    ...
    //   x_min
    for(int i = 0; i < point_lists[which_ploy].size(); i++){
        if(point_lists[which_ploy][i][0] < x_min){ // there is an intersection
            //find smallest index's loacation m, delete the indices, insert new indicies at m
            delete_indices.push_back(i);
        }
    }

    index = find_y_min_index(point_lists[which_ploy], delete_indices);
    deleteNum = delete_indices.size();
    

    if(deleteNum > 0){
        //cout << "x_min--found  " << deleteNum << "vertices to be deleted" << endl; 
        //cout << "first point is: "<< point_lists[which_ploy][index][0] << " " << point_lists[which_ploy][index][1];
        //cout <<"index at: " << index << endl;
        //string input;
        //cin >> input;
        
        int wrap_over_count = 0;
        int not_over_count = 0;
        if(index + deleteNum > point_lists[which_ploy].size()){
            wrap_over_count = deleteNum + index - point_lists[which_ploy].size();
            not_over_count = deleteNum - wrap_over_count;
        }
        else{
            wrap_over_count = 0;
            not_over_count = deleteNum;
        }
        
        //cout << "wrap_over_count:" << wrap_over_count << endl;
        //cout << "list_size: " << point_lists[which_ploy].size() << "index: " << index<< endl;
        
        //if indices exceeds list length, erase not over part, and then erase over part
        if(wrap_over_count > 0){
            for(int j = 0; j < not_over_count; j++)
                point_lists[which_ploy].erase(point_lists[which_ploy].begin()+index);
            for(int j = 0; j < wrap_over_count; j++)
                point_lists[which_ploy].erase(point_lists[which_ploy].begin());
        }
        else{
        ...
        //point_lists[which_ploy].erase(point_lists[which_ploy].begin()+index, point_lists[which_ploy].begin()+index+deleteNum);
        
        //cout << "list_size: " << point_lists[which_ploy].size() << endl;

        y_list.clear();
        for(int j = 0; j < lines_in_polygon[which_ploy].size(); j++){
            int y = x_intersect(lines_in_polygon[which_ploy][j], x_min);
            if(y != -1000){
                //cout << "x: " << x_min << "  y: " << y << endl;
                y_list.push_back(y);
            }
        }
        //x_min, sort y coordinates to insert clockwise
        sort(y_list.begin(),y_list.end());
        unique(y_list.begin(),y_list.end());
        for(int i = 0; i < y_list.size(); i++){
            temp_point[0] = x_min;
            temp_point[1] = y_list[i];
            point_lists[which_ploy].insert(point_lists[which_ploy].begin()+index-wrap_over_count+i, temp_point);
        } 
        point_lists[which_ploy] = unique_points(point_lists[which_ploy]);
        //cout << "after insertion, list_size: " << point_lists[which_ploy].size() << endl;
        //for(int i = 0; i < point_lists[which_ploy].size();i++){
        //    cout << point_lists[which_ploy][i][0] << " " << point_lists[which_ploy][i][1] << endl;
        //}
        //cin >> input;
    }

    make_polygons();//update lines info


    //    y_max
    delete_indices.clear();
    for(int i = 0; i < point_lists[which_ploy].size(); i++){
        if(point_lists[which_ploy][i][1] > y_max){ // there is an intersection
            //find smallest index's loacation m, delete the indices, insert new indicies at m
            delete_indices.push_back(i);
        }
    }
    index = find_x_min_index(point_lists[which_ploy], delete_indices);
    deleteNum = delete_indices.size();

    if(deleteNum > 0){
        //cout << "y_max--found  " << deleteNum << " vertices to be deleted" << endl; 
        //cout << "first point is: "<< point_lists[which_ploy][index][0] << " " << point_lists[which_ploy][index][1];
        //cout <<"index at: " << index << endl;
        //string input;
        //cin >> input;
        ...
        x_list.clear();
        for(int j = 0; j < lines_in_polygon[which_ploy].size(); j++){
            int x = y_intersect(lines_in_polygon[which_ploy][j], y_max);
            if(x != -1000)
                x_list.push_back(x);
        }
        //y_max, sort x coordinates to insert clockwise
        sort(x_list.begin(),x_list.end());
        unique(x_list.begin(),x_list.end());
        ...
        point_lists[which_ploy] = unique_points(point_lists[which_ploy]);
        //cout << "after insertion, list_size: " << point_lists[which_ploy].size();
        //for(int i = 0; i < point_lists[which_ploy].size();i++){
        //    cout << point_lists[which_ploy][i][0] << " " << point_lists[which_ploy][i][1] << endl;
        //}
        //cin >> input;
    ...
    delete_indices.clear();
    for(int i = 0; i < point_lists[which_ploy].size(); i++){
        if(point_lists[which_ploy][i][0] > x_max){ // there is an intersection
            //find smallest index's loacation m, delete the indices, insert new indicies at m
            delete_indices.push_back(i);
        }
    }
    index = find_y_max_index(point_lists[which_ploy], delete_indices);
    deleteNum = delete_indices.size();



    if(deleteNum > 0){
        //cout << "x_max--found  " << deleteNum << "vertices to be deleted" << endl; 
        //cout << "first point is: "<< point_lists[which_ploy][index][0] << " " << point_lists[which_ploy][index][1];
        //cout <<"index at: " << index << endl;
        //string input;
        //cin >> input;
        ...
        y_list.clear();
        for(int j = 0; j < lines_in_polygon[which_ploy].size(); j++){
            int y = x_intersect(lines_in_polygon[which_ploy][j], x_max);
            if(y != -1000){
                y_list.push_back(y);
            }
        }
    ...
    delete_indices.clear();
    for(int i = 0; i < point_lists[which_ploy].size(); i++){
        if(point_lists[which_ploy][i][1] < y_min){ // there is an intersection
            //find smallest index's loacation m, delete the indices, insert new indicies at m
            delete_indices.push_back(i);
        }
    }
    index = find_x_max_index(point_lists[which_ploy], delete_indices);
    deleteNum = delete_indices.size();


    if(deleteNum > 0){
        //cout << "y_min--found  " << deleteNum << "vertices to be deleted" << endl; 
        //cout << "first point is: "<< point_lists[which_ploy][index][0] << " " << point_lists[which_ploy][index][1];
        //cout <<"index at: " << index << endl;
        //string input;
        //cin >> input;
        ...
        x_list.clear();
        for(int j = 0; j < lines_in_polygon[which_ploy].size(); j++){
            int x = y_intersect(lines_in_polygon[which_ploy][j], y_min);
            if(x != -1000)
                x_list.push_back(x);
        }
...
//unique points in the list
vector<vector<int> > unique_points(vector<vector<int> > list){

    vector<vector<int> > temp_list;
    for(int i = 0;i < list.size();i++){
        if(is_in_list(list[i],temp_list) == false)
            temp_list.push_back(list[i]);
    }

    temp_list = delete_middle_point(temp_list);
    return temp_list;
}
        ...
        for (int j = 0; j < grid_height; j++){
            for (int i = 0; i < grid_width; i++){
                for(int k = 0; k < list[m].size();k++){
                    int tempx = list[m][k][0];
                    int tempy = list[m][k][1];
                    if(i == tempx && j == tempy){
                        lined_board[m][i][j] = 1;
                        //cout << "i: " << i << "  j: " << j << endl;
                    }
